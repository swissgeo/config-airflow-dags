...
SERVICE_NAME := config-airflow-dags
...
docker-network:
	@if ! docker network inspect config_airflow_dags_network >/dev/null 2>&1; then \
	echo "Creating network config_airflow_dags_network"; \
	docker network create config_airflow_dags_network; \
	else \
		echo "Network config_airflow_dags_network already exists"; \
	fi

.PHONY: docker-compose-up
docker-compose-up: docker-network ## Start local dependencies (postgres and iceberg)
	docker compose --env-file=${ENV_FILE} up --remove-orphans


.PHONY: docker-compose-down
docker-compose-down: ## Stop local dependencies (postgres and iceberg)
